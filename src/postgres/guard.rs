...
/// Task 6 этапа 1, реконнект вообще не обрабатывает — обработчик появится
/// этапом позже, на этапе 4. Не удалять как мёртвый код — реконнект-обработчик
/// станет её единственным потребителем.
