...
///
/// Граница держится ТОЛЬКО внутри каждого из этих мест, а не между ними:
/// между чтением флага на входе во внешний цикл и первым чтением флага
/// внутри сессии лежит preflight, установка соединения и запуск репликации
...
/// установления соединения, а не на `SHUTDOWN_POLL_INTERVAL`.
...
pub async fn run(config: Config, mut sink: Box<dyn Sink>) -> Result<(), PgcdcError> {
    ...
    loop {
        ...
        // Без этой проверки сигнал, пришедший пока БД недостижима, не был
        // бы замечен никогда: `stream_once` валится на preflight раньше,
        // чем доходит до собственной проверки флага внутри сессии, — а
        // обёртывающий цикл до сих пор его не читал вовсе.
        //
