...
///
/// Из них ТОЛЬКО первые два ограничены одной и той же величиной,
/// `SHUTDOWN_POLL_INTERVAL`, а не `ack_interval_ms` (тот управляет только
/// расписанием барьера) и не длиной паузы реконнекта: внутри сессии им
/// ограничено само чтение; нарезка бэкоффа проверяет флаг перед каждым
/// куском той же длины. Проверка на входе в проход внешнего цикла этой
/// границы НЕ держит: её период — целый оборот (сессия плюс пауза
/// реконнекта), а не `SHUTDOWN_POLL_INTERVAL`. Она не лишняя: нарезка
/// проверяет флаг ПЕРЕД каждым куском и ни разу ПОСЛЕ последнего, так что
/// сигнал, попавший именно в последний кусок, доходит только через эту
/// третью проверку на следующем обороте (подробности и цена такой
