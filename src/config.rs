...
pub struct Config {
    ...
    /// природы (сбой транспорта, недоступный сервер) не обнуляет его целиком,
    /// а лишь вычитает из накопленного время самого простоя, так что
    /// чередование с редкими посторонними сбоями эскалацию не отменяет —
    /// непрерывно занятый чужим потребителем слот всё равно исчерпает бюджет.
