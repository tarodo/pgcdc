...
1. **Guard перед стартом — два режима, а не одна проверка.** Холодный старт (процесс
   ...
   **Реконнект внутри процесса: полная сверка identity.** Если разрыв произошёл во время
   ...
   нашей in-memory durable-позицией. Расхождение — падаем громко, с обеими позициями в
   тексте ошибки. Слот **впереди** нашей durable-точки означает пропущенный WAL. Слот
   **позади** означает, что мы подтвердили больше, чем слот запомнил, — чужое
   вмешательство или наш баг; переигрывать молча нельзя, потому что неизвестно, что ещё
   разошлось. Автоматически «чинить» ни то, ни другое нельзя: решение о том, ронять ли
   гарантию доставки, принимает человек.
