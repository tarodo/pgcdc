...
Вывод: разрешён только `next_raw_event()`; реконнект пишем сами и рестартуем **с durable
позиции**, а не с принятой.
...
запрет и так безусловен, поэтому сценарий не воспроизводился. Этап 1 стоит закрыть одним
подтверждающим экспериментом.
...
1. **Guard перед каждым `start()`.** На отдельном обычном (не replication) соединении
   ...
   **Проверки существования НЕДОСТАТОЧНО, и это обязательная часть требования, а не
   опция.** Если между двумя запусками слот был удалён *и* создан заново — руками, чужим
   инструментом, чужим экземпляром нашего же демона — то `SELECT 1 FROM
   pg_replication_slots` пройдёт успешно, слот будет существовать, а WAL между нашей
   durable-позицией и новой позицией слота окажется пропущен. Это ровно тот исход, который
   измерен в пробе 4: там слот тоже существовал к моменту чтения потока, и всё равно
   строка `id=4000` не пришла никогда. Существование слота ничего не говорит о том, тот ли
   это слот и с той ли позиции он отдаёт.

   Поэтому guard состоит из двух шагов, и оба обязательны:

   1. `SELECT restart_lsn, confirmed_flush_lsn FROM pg_replication_slots WHERE slot_name = $1`
      на отдельном обычном (не replication) соединении. Пустой результат — слот отсутствует,
      падаем.
   2. Сверка `confirmed_flush_lsn` слота с нашей сохранённой durable-позицией. Расхождение —
      падаем громко, с обеими позициями в тексте ошибки. Слот **впереди** нашей durable-точки
      означает пропущенный WAL (сценарий выше). Слот **позади** означает, что мы подтвердили
      больше, чем слот запомнил, — это либо чужое вмешательство, либо наш баг; переигрывать
      молча нельзя, потому что мы не знаем, что ещё разошлось. Автоматически «чинить» ни то,
      ни другое нельзя: решение о том, ронять ли гарантию доставки, принимает человек.

   Остаточное окно TOCTOU (слот удалён между проверкой и `START_REPLICATION`) считаем
   пренебрежимым: оно требует удаления слота именно в этом промежутке.
2. **Запрещённые API.** Не использовать `next_event_with_retry()`, `check_connection_health()`,
   ...
   `next_raw_event()`; реконнект пишем сами, рестартуем с durable позиции и сбрасываем
   relation cache (DECISIONS Q19).
