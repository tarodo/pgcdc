...
Ретроспективно (этап 4): эксперимент закрыт без постановки — `check_reconnect` получил
вызывающего (`replication.rs`), но реальный `start_lsn` у нас всегда `0/0` (`start(None)`),
поэтому вопрос «что если `start_lsn` больше `confirmed_flush_lsn`» и не мог возникнуть.
