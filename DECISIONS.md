...
| Q15 | **TOAST-маркер `'u'`** (значение не менялось и лежит вне строки) → отдельное поле `unchanged_columns: ["bio"]`. Не опускаем колонку из `after`: опущенная колонка неотличима от удалённой при эволюции схемы, и потребитель затрёт значение на null. Не падаем: это штатное сообщение протокола. Появляется только для значений >~2 КБ после сжатия, на демо-таблице `users` не встретится. |
