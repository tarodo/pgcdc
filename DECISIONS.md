...
- `event_index`: this event's position within its transaction, starting at
  ...
  writes its rows as one WAL record, and a single `TRUNCATE` naming several
  tables is also one WAL record — either way, every event produced from
  that one record carries the same `lsn` (Q35).
