...
транспорта, недоступный сервер) не обнуляет его целиком, а лишь вычитает из
накопленного время самого простоя — чередование с редкими посторонними
сбоями эскалацию не отменяет, слот, занятый чужим потребителем навсегда,
всё равно исчерпает бюджет. Полностью счётчик закрывает только успешный
