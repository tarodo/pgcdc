...
SIGTERM/SIGINT — принятое доводится до барьера, подтверждается, и процесс
завершается с нулевым кодом возврата, — и переживание жёсткого перезапуска
(SIGKILL и повторный запуск с той же позиции слота).
